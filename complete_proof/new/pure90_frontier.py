...
CATALOG=json.loads(Path(__file__).with_name('pure_catalogue.json').read_text())['types']
...
def run():
 q=90;units=10;result={}
 for iso in (False,True):
  if iso:
   types=[t for t in CATALOG if F(t['min_kernel'])>0]
  else:
   types=[t for t in CATALOG if t['v']<=7 and t['u']+(15-t['v']+1)//2<=units]
  ...
  print('pure90','isolate'if iso else'noisolate','types',len(types),'assemblies',assemblies,'normaboveB',len(old),'afterprojection',len(surv),flush=True)
 ...
 Path(__file__).with_name('pure90_frontier.json').write_text(json.dumps(result,indent=2)+'\n');return result
