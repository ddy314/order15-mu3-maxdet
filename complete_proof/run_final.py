...
Default: rerun the original audit, all previous catalogues, and all new
candidates with two independent exhaustive column algorithms.
--cached-baseline: reuse only the four hash-checked previous catalogues;
all new computations and the original audit are still rerun.
"""
...
def check_manifest(name):
    ...
    for rel,digest in json.loads(path.read_text()).items():
        if hashlib.sha256((ROOT/rel).read_bytes()).hexdigest()!=digest:
            raise RuntimeError('Source/snapshot hash mismatch: '+rel)

def main():
    if not __debug__:
        raise RuntimeError('Assertions must remain enabled; do not use python -O.')
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--cached-baseline',action='store_true')
    a=p.parse_args();check_manifest('final_source_sha256.json')
    if a.cached_baseline:
        check_manifest('catalogue_sha256.json')
    ...
    summary={'completed':True,'all_steps_passed':True,'cached_baseline_catalogues':a.cached_baseline,
