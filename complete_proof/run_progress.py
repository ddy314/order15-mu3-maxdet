...
All catalogues are regenerated by default. --cached retains the delivered
catalogues, verifies their hashes, and reruns the downstream arguments.
Exit 2 means that genuine global proof obligations remain; --audit-only asks
only whether the documented partial proof and its checks completed.
"""
...
def check_manifest(path:Path)->None:
 if not path.exists():raise FileNotFoundError(f'Missing manifest: {path}')
 for relative,digest in json.loads(path.read_text()).items():
  actual=hashlib.sha256((ROOT/relative).read_bytes()).hexdigest()
  if actual!=digest:raise RuntimeError(f'Hash mismatch: {relative}')
...
def main()->int:
 ...
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--audit-only',action='store_true');p.add_argument('--cached',action='store_true');p.add_argument('--include-baseline',action='store_true');args=p.parse_args()
 check_manifest(ROOT/'provenance/source_sha256.json')
 if args.cached:check_manifest(ROOT/'provenance/catalogue_sha256.json')
 ...
 if not args.cached:commands.extend((name,[sys.executable,str(NEW/name)])for name in CATALOGUES)
 ...
 ledger=make_ledger()
 summary={'python':sys.version,'platform':platform.platform(),'used_cached_catalogues':args.cached,'baseline_replayed_in_this_run':args.include_baseline,'steps':steps,'all_executed_steps_passed':True,'seconds':time.monotonic()-start,'newly_closed':ledger['newly_closed'],'remaining':ledger['remaining_shells'],'global_maximality_verified':ledger['global_maximality_verified']}
