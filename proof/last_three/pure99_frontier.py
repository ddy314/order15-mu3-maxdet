...
CATALOG=json.loads((Path(__file__).resolve().parents[1]/'new'/'pure_extended.json').read_text())['types']
...
def run():
 ...
 for iso in (False,True):
  ...
  print('pure99','isolate'if iso else'noisolate','types',len(types),'assemblies',assemblies,'normaboveB',len(old),'afterprojection',len(surv),flush=True)
 ...
 Path(__file__).with_name('pure99_frontier.json').write_text(json.dumps(result,indent=2)+'\n');return result
